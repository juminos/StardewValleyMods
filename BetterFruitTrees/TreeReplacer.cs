...
namespace WilderTrees
{
    ...
    {
        public static void ReplaceTree(ModConfig config, IMonitor monitor)
        {
            foreach (GameLocation location in Game1.locations)
            {
                var treeList = location.terrainFeatures.Pairs
                    .Where(pair => pair.Value is Tree)
                    .ToArray();
                foreach (KeyValuePair<Vector2, TerrainFeature> pair in treeList)
                {
                ...
                Console.WriteLine($"Weighted fruit tree list count: {weightedFruitTreeList.Count}");
                    ...
                    Console.WriteLine($"Processing wild tree keyword: {wildTreeKeyword.Keyword}");
                                ...
                                else
                                {
                                    ...
                                    return;
                                    ///////////////////
                                    ///Wild tree data cannot be retrieved this way
                                    ///////////////////
        ...
        public static void ResetTrees(IMonitor monitor, ModConfig config)
        {
            foreach (GameLocation location in Game1.locations)
            {
