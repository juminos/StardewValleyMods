...
namespace FrenshipRings.Caching;
    ...
    where TKey : notnull
{
    ...
    {
#if DEBUG
        ModEntry.SMonitor.Log($"{this.cache.Count} in hot cache, {this.stale.Count} in the stale cache before swap.");
#endif
        this.stale.Clear();
    ...
    private void OnTimerCallback(object? state)
    {
        ...
        {
            ModEntry.SMonitor.Log($"[AtraBase] Cache swap failed., {ex}");
        }
    }
