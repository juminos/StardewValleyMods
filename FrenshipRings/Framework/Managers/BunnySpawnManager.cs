...
internal sealed class BunnySpawnManager : IDisposable
{
    ...
    internal List<Bush>? GetTrackedBushes()
    {
        ...
        {
            ModEntry.SMonitor.Log($"Refreshing watcher on {farmer.Name}");
            watcher = new(ModEntry.SMonitor, farmer.currentLocation);
    ...
    private void OnWarp(object? sender, WarpedEventArgs e)
    {
        ...
        {
            ModEntry.SMonitor.Log($"Warp event raised, not for us", LogLevel.Info);
            return;
        }
    ...
    private sealed class LargeObjectListener : IDisposable
    {
        ...
        {
            this.monitor.Log($"(Bunny Ring) Changing bushwatcher {this.location?.NameOrUniqueName} -> {newLocation.NameOrUniqueName}");
        ...
        private void OnValueAdded(LargeTerrainFeature feature)
        {
            ...
            {
                this.monitor.Log($"Bush added at {bush.Tile}");
        ...
        private void OnValueRemoved(LargeTerrainFeature feature)
        {
            ...
            {
                this.monitor.Log($"Bush removed at {bush.Tile}");
            }
        }
    }
}
