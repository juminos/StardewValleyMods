        ...
        public class Farmer_MovePosition_Patch
        {
            public static bool Prefix(Farmer __instance, GameLocation currentLocation)
            {
                ...
                foreach (var d in __instance.movementDirections)
                {
                    ...
                    {
                        if (nextPosition.Intersects(kvp.Value.GetBoundingBox()))
                        {
                            ...
                            {
                                currentLocation.playSound("hammer", null, null, StardewValley.Audio.SoundContext.Default);
                                currentLocation.destroyObject(kvp.Key, __instance);
                            }
                            else if (kvp.Value.Name.Contains("Twig"))
                            {
                                kvp.Value.Fragility = 2;
                                currentLocation.playSound("axchop", null, null, StardewValley.Audio.SoundContext.Default);
                                Game1.createRadialDebris(currentLocation, 12, (int)kvp.Value.TileLocation.X, (int)kvp.Value.TileLocation.Y, Game1.random.Next(4, 10), false, -1, false, null);
